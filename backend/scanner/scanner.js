...
const BACKEND_INVENTORY_DB = path.join(REPO_ROOT, "backend", "inventory.db");
const ROOT_INVENTORY_DB = path.join(REPO_ROOT, "inventory.db");
/** Align with ``backend/db/inventory_db.py``: prefer ``backend/inventory.db`` when present. */
const DB_PATH =
    process.env.INVENTORY_DB_PATH ||
    (fs.existsSync(BACKEND_INVENTORY_DB) ? BACKEND_INVENTORY_DB : ROOT_INVENTORY_DB);
...
async function main() {
  ...
  try {
    for (let i = 0; i < dealers.length; i++) {
      ...
      try {
        const vehicles = await runDealer(browser, dealer, { profile: "default", scrollLazyLoad: true });
        if (vehicles.length > 0) {
          successful.push({
            name: dealer.name || dealer.dealer_id,
            url: dealer.url,
    ...
    if (successful.length > 0) {
      ...
      for (const result of successful) {
        console.log(`  • ${result.name}: ${result.count} vehicles`);
      }
    }

    if (failed.length > 0) {
      console.log(`\n✗ Failed or empty (${failed.length}):`);
    ...
    console.log("\n" + "=".repeat(80));
    console.log(`Total: ${successful.length} successful, ${failed.length} issues`);
    console.log("=".repeat(80) + "\n");
  } finally {
    await browser.close();
  }
