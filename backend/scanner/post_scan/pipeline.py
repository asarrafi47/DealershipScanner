...
def run_listing_gap_fill_stage(vins: list[str]) -> dict[str, Any]:
    """See ``backend.scanner.post_scan.gap_fill``."""
...
def post_dict_enrich_env_enabled() -> bool:
    """EPA epa_master enrichment fills spec gaps (transmission, cylinders, MPG, etc.). Default on."""
...
def run_dictionary_enrich_for_vins(vins: list[str]) -> dict[str, Any]:
    """
    Fill mechanical spec gaps from epa_master for each scanned VIN.
